...
func _on_level_up() -> void:
	...
	# Тут можно добавить бонусы для игрока (например, увеличение max_health)
	max_health += 10
	health = max_health  # Лечим при повышении уровня
	print("❤️ Максимальное здоровье увеличено до ", max_health)
	leveled_up.emit()
